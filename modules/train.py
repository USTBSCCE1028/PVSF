...
class EETrainer(BaseTrainer):
    ...
    def train(self):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            #存loss
            df_ot = pd.DataFrame(loss_l)
            df_ot.columns = ['epoch', 'loss']
            df_ot.to_csv(f'./{self.args.Select}_{self.args.use_prompt}_train_loss.csv', index=False)
            pbar.close()
            self.pbar = None
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    #存loss
                    loss_dev.append([epoch, total_loss])
                ...
                # evaluate done
                pbar.close()
                sk_result = sk_classification_report(y_true=true_labels, y_pred=pred_labels, labels=list(self.re_dict.values()), target_names=list(self.re_dict.keys()), digits=4)
                self.logger.info("%s\n", sk_result)
                ...
                #存loss
                df_ot = pd.DataFrame(loss_dev)
                df_ot.columns = ['epoch', 'loss']
                df_ot.to_csv(f'./{self.args.Select}_{self.args.use_prompt}_dev_loss.csv', index=False)
        self.model.train()
...
class NERTrainer(BaseTrainer):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    self.step += 1
                    batch = (tup.to(self.args.device) if isinstance(tup, torch.Tensor) else tup for tup in batch)
                    attention_mask, labels, logits, loss, idx = self._step(batch, mode="train")
                    avg_loss += loss.detach().cpu().item()
                    #存loss
                    loss_l.append([epoch, avg_loss])
                    loss.backward()
                    self.optimizer.step()
                    self.scheduler.step()
                    self.optimizer.zero_grad()
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    step += 1
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)  # to cpu/cuda device
                    attention_mask, labels, logits, loss, idx = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()
                    loss_dev.append([epoch, total_loss])
    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    ...
                    for row, mask_line in enumerate(input_mask):
                        ...
                        if self.args.labeled:
                            y_labeled.append([idx[row], true_predict])
                    pbar.update()
                # evaluate done
                pbar.close()
                results = classification_report(y_true, y_pred, digits=4)
                ...
                if self.args.labeled:
                    with open(self.args.labeled_data_path, 'r', encoding='utf-8') as file:
                        data = json.load(file)
                    for idx, labels in y_labeled:
                        data[idx]['entity_predict'] = labels
                    with open(self.args.labeled_data_path, 'w', encoding='utf-8') as file:
    ...
    def _step(self, batch, mode="train"):
        if self.args.use_prompt:
            input_ids, token_type_ids, attention_mask, NER_labels, images, aux_imgs, seq_input_ids, seq_token_type_ids, seq_attention_mask, idx = batch
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                            NER_labels=NER_labels, images=images, aux_imgs=aux_imgs,
            ...
            input_ids, token_type_ids, attention_mask, NER_labels, idx = batch
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, NER_labels=NER_labels, images=images, aux_imgs=aux_imgs,
                                seq_input_ids=seq_input_ids, seq_token_type_ids=seq_token_type_ids, seq_attention_mask=seq_attention_mask)
        logits, loss = output.logits, output.loss
        return attention_mask, NER_labels, logits, loss, idx
...
class METrainer(BaseTrainer):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    attention_mask, labels, logits, loss, idx = self._step(batch,
    ...
    def _step(self, batch, mode="train"):
        if self.args.use_prompt:
            input_ids, token_type_ids, attention_mask, ROLE_labels, images, aux_imgs, seq_input_ids, seq_token_type_ids, seq_attention_mask, dec_table_attention_mask, idx = batch
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                ROLE_labels=ROLE_labels, images=images, aux_imgs=aux_imgs,
                                seq_input_ids=seq_input_ids, seq_token_type_ids=seq_token_type_ids,
                                seq_attention_mask=seq_attention_mask, dec_table_attention_mask=dec_table_attention_mask)
        else:
            images, aux_imgs = None, None
            seq_input_ids, seq_token_type_ids, seq_attention_mask = None, None, None
            input_ids, token_type_ids, attention_mask, ROLE_labels, idx = batch
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                ROLE_labels=ROLE_labels, images=images, aux_imgs=aux_imgs,
                                seq_input_ids=seq_input_ids, seq_token_type_ids=seq_token_type_ids,
                                seq_attention_mask=seq_attention_mask)
        logits, loss = output.logits, output.loss
        return attention_mask, ROLE_labels, logits, loss, idx
